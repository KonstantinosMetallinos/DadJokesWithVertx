...
class JokeVerticleTest {
    ...
    void test_fetchJoke_withData() throws InterruptedException {
        ...
        JokeConfigRecord localConfig = new JokeConfigRecord("localhost", TEST_PORT_WITH_DATA, ENDPOINT_URI, JOKE_INTERVALS_SECONDS);
        ...
        vertx.eventBus().request(DAD_JOKE_GET, config)
                ...
                .onFailure(f -> Assertions.fail()); // We expect a 200 Response.
    ...
    void test_fetchJoke_withHttpErrorResponse() throws InterruptedException {
        // Given
        VertxTestContext blocker = new VertxTestContext();
        JokeConfigRecord localConfig = new JokeConfigRecord("localhost", TEST_ERROR_PORT, ENDPOINT_URI, JOKE_INTERVALS_SECONDS);

        jokeVerticle.setHttpRequest(localConfig);

        // When
        vertx.eventBus().request(DAD_JOKE_GET, config)
                .onSuccess(s -> Assertions.fail()) // We expect a non 2XX Response.
